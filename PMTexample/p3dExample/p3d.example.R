...
latlon <- c(5265111.03019346, 816499.40129895)
...
# or also specific profile orientations can be set, by providing a vector of degrees. e.g. c(12,14,16), E = 0, counterclockwise
deg = NA #c(0,2,4,6,8)

# spatial resolution in [m]
analysis.resolution <- 5

# specify the path, name and extension of a raster (lidar) file
#dem_1.path.name.extension <- c("H:/GIS/DEMs/5m", "naf_5m_EPSG32632___", ".tif")
dem_1.path.name.extension <- c("H:/PMTexample/sampleData", "dem5m", ".tif")

# specify the path and name of a map (polygons)
#map_1.path.name <- c("H:/GIS/PhD/maps/flagg", "terraces")
map_1.path.name <- c("H:/PMTexample/sampleData",
                     "terraces")
...
map1.infield.outfield <- c("Bez", "Bez",
                           "Niveau", "Niveau",
                           "Color", "Color",
                           "terraceID", "terraceID")
...
#eval(parse(text = readtext("../Functions.R/pt1/linear.reference.points.R",verbosity = 0)[[2]])) # OLD TBD
