...
setwd("H:/PMTexample/exampleProfile") # Win
...
search.radius <- 800 # 6500 in FLAGG 1
...
profile.path.name <- c("profil",
                       "profil")

# Raster(s) - one or more rasters can be used. rasters should be uniquely named.
dem_1.path.name.extension <- c("H:/PMTexample/sampleData", 
                               "dem5m", 
                               ".tif")
...
# 8. load, clip to buffer, extract lidar and linear reference a point shapefile (optional)
  ...
  # linear referencing
    if (length(get(paste0("points_",i))$geometry) != 0) { # if there are features present, project them
      assign(
...
points.list <- function(){
  ...
  } else {points <- as.data.frame(points_1.projected)}
  if (parameters$points.count > 2) {
    for (i in 3:parameters$points.count) {
