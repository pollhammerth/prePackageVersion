...
prepare.data <- function(data = data, x = x[1], y = y[1], z = z[1], field = field[1]) {
  ...
  terrace <- unit.names(data, field[1])
  if (length(field) > 1) {
    for (i in 2:length(field)) {
      terrace <- c(terrace, unit.names(data, field[i]))
    }} else {}
  ...
  for (i in 1:length(y)) {
    for (j in 1:length(field)) {
      for (k in 1:length(unit.names(data, field[j]))) {
        # write object
        eval(parse(text = paste0(y[i], ".", unit.names(data, field[j])[k],
                                 " <<- subset(data, data$", field[j], " == '", unit.names(data, field[j])[k], "')")))
        # rename defined y column to "y" in created object
        eval(parse(text = paste0("names(", y[i], ".", unit.names(data, field[j])[k],
                                 ")[names(",y[i], ".", unit.names(data, field[j])[k], ") == '", y[i], "'] <<- 'y'")))
      }}}
