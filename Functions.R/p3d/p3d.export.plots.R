...
{  
  ...
  dir.create(paste0(location.name,"/",out.dir,"/",profile.dir,"/",dir), showWarnings = FALSE)
  ...
  for (i in 1:length(select.profile.by.degree)) {
    ...
    if (plot.function == "p3d.plot.data.allUnits") {
      png(filename=paste0(location.name,"/",out.dir,"/",profile.dir,"/",dir,"/","id.",id,".png"),width=16,height=8,units="cm",res=300)
    ...
    if (plot.function == "p3d.plot.model.allUnitsAnnotated") {
      png(filename=paste0(location.name,"/",out.dir,"/",profile.dir,"/",dir,"/","id.",id,".png"),width=16,height=16,units="cm",res=300)
    ...
    if (plot.function == "p3d.plot.lidar") {
      png(filename=paste0(location.name,"/",out.dir,"/",profile.dir,"/",dir,"/","id.",id,".png"),width=16,height=8,units="cm",res=300)
      p3d.plot.lidar(profile.data[[id]], 
                             main = paste0("ID ",id," | ",
                                           select.profile.by.degree[i], " degrees"),
                             ...
      )
