...
pmt.bin <- function(data, interval = 500, value = "median", mode = "bin", idfield = "map.terrace", cth = NA, sth = NA){
  ...
  #### calculate statistics for polygon ids
    ...
    c <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],length(y))
    for (i in 2:length(as.character(unique(data[[idfield]])))) {
      c <- c(c,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], length(y)))
    }
    s <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],round(sd(y),1))
    for (i in 2:length(as.character(unique(data[[idfield]])))) {
      s <- c(s,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], round(sd(y),1)))
    }
    id <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),], as.character(unique(data[[idfield]]))[i])
    for (i in 2:length(as.character(unique(data[[idfield]])))) {
      id <- c(id,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], as.character(unique(data[[idfield]]))[i]))
    }
    
    
    # calculate y values for each polygon
    if (value == "median") {
    y <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],median(y))
    for (i in 2:length(as.character(unique(data[[idfield]])))) {
      y <- c(y,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], median(y)))
    }
    } else if (value == "min") {
      y <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],min(y))
      for (i in 2:length(as.character(unique(data[[idfield]])))) {
        y <- c(y,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], min(y)))
      }
    } else if (value == "max") {
      y <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],max(y))
      for (i in 2:length(as.character(unique(data[[idfield]])))) {
        y <- c(y,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], max(y)))
      }
    } else if (value == "mean") {
      y <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],mean(y))
      for (i in 2:length(as.character(unique(data[[idfield]])))) {
        y <- c(y,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),], mean(y)))
      }
    }
    
    # calculate polygon-centered x values
    x <- with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[1]),],(max(x)-min(x))/2+min(x))
    for (i in 2:length(as.character(unique(data[[idfield]])))) {
      x <- c(x,with(data[which(data[[idfield]] == as.character(unique(data[[idfield]]))[i]),],(max(x)-min(x))/2+min(x)))
    }
