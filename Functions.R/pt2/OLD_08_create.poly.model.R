...
#### Content:
# create.poly.model()       # [1]datatype,[2]i(terrace.no),[3]y,[4]order,[5]interval="full","set","use",[6]use.int.no,[7]x,[8]plot
# elevated.model()          # additional argument [9]elevate=0
#### Benoetigt:
# model.intervals
#### Output:
...
create.poly.model<-function(datatype="td",i=1,y="y",order=3,interval="full",use.int.no=1,x="x",plot=FALSE,col="black",conf=FALSE,save=FALSE){
...
elevated.model <- function(datatype="m500",i=2,y="median",order=3,interval="full",use.int.no=1,x="x",plot=TRUE,col="black",conf=FALSE,save=FALSE,elevate=0) {
    name<-paste0(datatype,sub("td","",td[i]),'$',y)
    temp<-paste0(name,' <<- ',name,' + ',elevate);eval(parse(text=temp)) # Wert addieren
    create.poly.model(datatype,i,y,order,interval,use.int.no,x,plot,col,conf,save)
    temp<-paste0(name,' <<- ',name,' - ',elevate);eval(parse(text=temp)) # Wert addieren
}
