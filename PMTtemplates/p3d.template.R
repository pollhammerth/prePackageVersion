...
variable.params <- list("Schiener_M",5281027.91360976, 490284.77543343,13500,20)
variable.params <- list("Schiener_L",5281027.91360976, 490284.77543343,26500,40)
variable.params <- list("Stadler_M",5264776.46673073, 459347.56078135,12500,20)
variable.params <- list("Tromsberg_M",5258094.10652849, 443443.94972253,9000,10)
variable.params <- list("Tromsberg_M_TEST",5258094.10652849, 443443.94972253,9000,100) # test purposes
...
# specify the path, name and extension of a raster (lidar) file (multiple raster files supported)
...
dem_2.path.name.extension <- c("H:/GIS/PhD/maps/flagg/baseNAB/nagra/nab1235",
                               "thds_130813", # Top HDS NAB 12-35
                               ".tif")
dem_3.path.name.extension <- c("H:/GIS/PhD/maps/flagg/baseNAB/nagra/nab1235",
                               "bhds_130813", # Base HDS NAB 12-35
                               ".tif")
dem_4.path.name.extension <- c("H:/GIS/PhD/maps/flagg/baseNAB/nagra/nab1235",
                               "btds_130813", # Base TDS NAB 12-35
                               ".tif")
dem_5.path.name.extension <- c("H:/GIS/PhD/maps/flagg/baseNAB/nagra/nab1235",
                               "isoflu_130813", # Erosionsbasis Fluesse NAB 12-35
                               ".tif")
dem_6.path.name.extension <- c("H:/GIS/PhD/maps/flagg/baseNAB/nagra", 
                               "bqu_140715g", # Base Quaternary (Felsmodell) NAB 14-02
                               ".tif")
dem_7.path.name.extension <- c("H:/GIS/PhD/maps/flagg/Sternai2012",
                               "QuaternaryUpliftGradient", # Quaternary erosion-deposition uplift gradient, Sternai et al. (2012)
                               ".tif")



# specify the path and name of a map (polygons) only one map supported
...
#################################################################### test start  
  # dem.count
  dem.count = length(ls(pattern = "dem_")[seq(1, by = 2, len = length(ls(pattern = "dem_")))]),
  # dem.names
  dem.names = 
    eval(expression(
...
###################################################################### test end  
  # dem.names
  #dem.names = dem_1.path.name.extension[2], # if "test" is not used, uncomment this single line!
  # map.names
